...
class EffBlock(nn.Module):
    def __init__(self, in_channels, out_channels, expand_radio=2):
        super(EffBlock, self).__init__()
        
        mid_channels = in_channels // expand_radio
        ...
        self.depthwise_1x3 = nn.Sequential(
            nn.Conv2d(mid_channels, mid_channels, 
                     kernel_size=(1, 3), stride=1, padding=(0, 1), groups=mid_channels, bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.LeakyReLU(negative_slope=0.1, inplace=True)
        )
        ...
        self.depthwise_3x1 = nn.Sequential(
            nn.Conv2d(mid_channels, mid_channels, 
                     kernel_size=(3, 1), stride=1, padding=(1, 0), groups=mid_channels, bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.LeakyReLU(negative_slope=0.1, inplace=True)
        )
...
class EFFNet(nn.Module):
    def __init__(self, model_config):
        ...
        self.effnet_block1 = EffBlock(32, 64, expand_radio=2)  
        self.effnet_block2 = EffBlock(64, 128, expand_radio=2)  
        self.effnet_block3 = EffBlock(128, 256, expand_radio=2)  
