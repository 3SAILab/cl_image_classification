...
class AnchorTargetCreator(object):
    def __init__(self, n_sample=256, pos_iou_thresh=0.7, neg_iou_thresh=0.3, pos_ratio=0.5):
        ...
        self.pos_ratio = pos_ratio # 正样本占比
...
class FasterRCNNTrainer(nn.Module):
    def __init__(self, model_train, optimizer):
        ...
        self.rpn_sigma = 1  # RPN回归损失用的平滑参数
        self.roi_sigma = 1  # RoI回归损失用的平滑参数
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        for i in range(n):
            bbox = bboxes[i]  # 第i张图片的真实框
            label = labels[i] # 真实类别
            rpn_loc = rpn_locs[i]  # 预测偏移量
            rpn_score = rpn_scores[i]  # 预测分数
            roi = rois[i] # 候选框
            ...
            sample_rois.append(torch.Tensor(sample_roi).to(device))
            sample_indexes.append(torch.ones(len(sample_roi), device=device) * roi_indices[i][0])
            gt_roi_locs.append(torch.Tensor(gt_roi_loc).to(device))
            gt_roi_labels.append(torch.Tensor(gt_roi_label).to(device).long())
...
#     pred_loc = torch.randn(n_samples, 4) * 0.5  # 随机生成，稍微加点噪声
#     print("预测偏移量 pred_loc:\n", pred_loc)

#     gt_loc = torch.randn(n_samples, 4) * 0.5
#     print("\n真实偏移量 gt_loc:\n", gt_loc)
...
#     regression_loss = _fast_rcnn_loc_loss(pred_loc, gt_loc, gt_label, sigma)
#     print("回归损失 regression_loss:\n", regression_loss)
