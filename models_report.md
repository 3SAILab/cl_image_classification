...
### 0.前期工作
#### 1.做了什么，为什么做？
- 传统的目标检测方法陷入停滞，彼时CNN在图像分类上的高精度让该团队想到是否能将CNN推广到目标检测上。
#### 2.亮点
- 使用CNN提取特征
- 提出“两阶段检测”框架（候选区域生成+目标分类定位）
#### 3.搞懂
- 传统的目标检测如何实施，有什么局限性？  
滑动窗口（为了找到目标物体的位置，先把所有地方都找一遍）+手工提取特征+分类器（例如SVM）  
局限：①滑动窗口的计算量太大，近乎是暴力搜索。  
②手工提取特征既复杂精度又不高  
③难以处理多分类（每一个类别都要手工提取特征+分类器）
- R-CNN的检测框架是什么？为什么好？  
①候选区域生成：使用selective search算法，找出有可能的位置（不像之前所有都找）  
②特征提取：用预训练的CNN提取特征（比手工提取精度更高，而且操作更简单）  
③分类：SVM给每个类别分类（不像之前提取特征+SVM不分开，这里只用提取一遍特征然后不断分类就行）  
④精修位置：线性回归调整框的位置    
- 为什么不用softmax做分类？  
