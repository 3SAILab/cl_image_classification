...
左编码器，右解码器（多了掩码）  
代码详解：<https://nlp.seas.harvard.edu/2018/04/03/attention.html>
### 二、重点
...
#### 5.掩码（Mask）
- padding mask:每个批次的输入序列长度不同，为了对其将短的填充0（太长则截断），这些0值无意义，所以给它们加上 $-\infty$ ，这样再经过softmax后值就趋近于0.
...
- 编码器：6层堆叠，每层2个子层：多头注意力+FFN。每个子层后都有残差+层归一化。
- 解码器：6层堆叠，每层3个子层：掩码多头注意力+多头注意力+FFN，每个子层后都有残差+层归一化。  
其中掩码多头注意力就是比普通多头注意力多了个mask；多头注意力与编码器有一点不同，KV来自编码器的输出，Q来自上一层掩码多头注意力的输出。
### 三、注意点
#### 1.模型输入输出数据（不考虑模型参数）
- 训练时传入src（源序列）,tgt（目标序列）（根据原论文的模型应该还传入mask，但我在代码中新增了构建mask的代码，为了简单化，后面有影响再改回来）。这里的src,tgt都是经过处理的，具体处理步骤：原句->分词->根据词表变成相应编码->加<>（序列结束符和开始符）->padding。  
输出是概率分布序列，注意需要调用模型的generator函数，否则输出的只是解码器的输出，无法计算loss。
- 推理时传入src,tgt（此时的tgt只有< sos >,因为预测输出是自回归生成），注意只调用decoder。
输出是概率分布序列（序列长度与tgt相同，这个很重要！！！），因此它是一个词一个词生成的，需要把结果拼接。
#### 2.序列开始符和结束符（< sos >,< eos >）
- 关于src：对于一般预测任务只用加< eos >，用于和padding的后续长度隔开，告诉模型在这里结束；对于分类任务，需要加< sos >，因为需要一个全局总结值。
...
从公式能看出，它用了标准正态分布的累计分布函数，相当于：  
当 $x \to +\infty$ ，$\Phi(x) \to 1$ → 完全通过  
当 $x \to -\infty$ ，$\Phi(x) \to 0$ → 几乎抑制  
当 $x = 0 $ ，$\Phi(0) = 0.5$ → 一半概率通过  
（个人理解，在输入接近0时给了一定概率，且这个概率近似高斯分布，在bn等处理过后的输入都符合高斯分布，两者相互配合效果较好；relu的梯度不是0就是1，但gelu是慢慢变化的，没有那么极端，在反向传播过程中作用也更好。）  
公式：  
...
# R-CNN
### 一、网络框架
![Alt](https://i-blog.csdnimg.cn/blog_migrate/af199568fe6bb2cce05c6899a72508bf.png)
### 二、重点
#### 1.使用CNN提取特征
- 传统目标检测？滑动窗口（为了找到目标物体的位置，先把所有地方都找一遍）+手工提取特征+分类器（例如SVM）  
局限：①滑动窗口的计算量太大，近乎是暴力搜索。  
②手工提取特征既复杂精度又不高  
③难以处理多分类（每一个类别都要手工提取特征+分类器）
- 直接用预训练的CNN，比手工提取精度更高，而且操作更简单。
#### 2.提出“两阶段检测”框架
- 候选区域生成+目标分类定位（特征提取+分类+精修位置）
- 候选区域生成：使用selective search算法，找出有可能的位置（不像之前所有都找）
- 分类：SVM给每个类别分类（不像之前提取特征+SVM不分开，这里只用提取一遍特征然后不断分类就行）。不用softmax是因为：  
①SVM能基于特征独立训练，而softmax一般是和CNN端到端的训练。  
②SVM二分类能适应复杂背景，softmax输出概率容易被影响。
- 精修位置：线性回归调整框的位置。

# Fast R-CNN
### 一、网络架构
![Alt](https://i-blog.csdnimg.cn/direct/e3708d9591ab4c4e83354d3aaec8bf3e.png)
### 二、重点
#### 1.共享卷积特征图
- RCNN选出候选区域后，将每一个图像单独剪裁送入CNN。Fast RCNN直接将整张图送进去。
#### 2.ROT Pooling层
- CNN后要将每个候选框的特征送进FC，但它们大小不同一，因此先经过这个层使它们的大小统一。
- 池化到固定尺寸7x7，然后展平。
#### 3.多任务损失函数
...
# 补充
（一些学习过程中遇到的问题，记录解决过程）
### 1.关于md上传github后公式无法显示的问题
- 目前github已经支持部分公式的渲染，但格式非常严格，可能在本地可以显示但上传后就无法显示了。
- 主要格式有两种：  
①公式行($...$)  
注意前后两端必须有空格，且符号和中间的公式不能有空格（比如$ ... $这样就不行）  
举例： $\frac{1}{a}$ (见源码)  
②公式块($$...$$)  
注意最好是分行写，与其他内容上下各隔一行，同样符号与中间内容相接处不能有空隙。  
举例：  
...
- 似乎（）内的前后两端无所谓有无空格，猜测是与字符连一起时会被github识别成字符，所以要隔开。