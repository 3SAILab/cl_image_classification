...
### 0.前期工作
#### 1.做了什么，为什么做？
- 推理时间受中间特征图内存流量的限制，但现有的抵消内存流量的方法会损失精度，于是他们提出了一种不会损失精度的降低内存流量的办法。
#### 2.亮点
- 提出核心指标CIO（卷积输入/输出）
- 稀疏化连接模式
- 通道平均加权
- 倒置过渡模块
#### 3.搞懂
- 稀疏化连接模式？DenseNet连接太多，导致后期输入暴涨，他们的连接模式是让k层连接到$k-2^n$层（只考虑能整除$2^n$层的密集连接),例如第4层只连接0，2，3层。
- 通道平均加权？为了平衡输入/输出的通道数，避免搬运过多的数据但只进行少量的计算，简单理解就是输入通道数多的层输出通道数也多，但它们之间是平衡的。  
- 倒置过渡模块？DenseNet中采用“Conv1x1+2x2平均池化”，该模型调整为先分别平均池化和最大池化，拼接之后用一个Conv1x1完成通道压缩。为了不让卷积层的计算量过高。
### 一、网络架构
